...
size_t ByteStream::write(const string &data) {
    ...
    size_t remaining_capacity = _capacity - _buffer.size();
    size_t write_size = min(data_size, remaining_capacity);
    _buffer += data.substr(0, write_size);
...
string ByteStream::peek_output(const size_t len) const {
    size_t copy_size = min(len, _buffer.size());
    return _buffer.substr(0, copy_size);
}

//! \param[in] len bytes will be removed from the output side of the buffer
void ByteStream::pop_output(const size_t len) {
    size_t pop_size = min(len, _buffer.size());
    _buffer = _buffer.substr(pop_size);
